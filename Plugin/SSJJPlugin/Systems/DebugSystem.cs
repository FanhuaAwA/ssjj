...
namespace UnityEngine.Components
{
    public class DebugSystem
    {
        ...
        public static void LogV(string name, Vector3 pos, float hp, float maxHp, int team)
        {
            Log("V", $"{name} Pos=({pos.x:F1},{pos.y:F1},{pos.z:F1}) HP={hp}/{maxHp} Team={team}");
        }

        public static void LogAim(string target, float dist, string bone)
        {
            Log("C", $"Lock:{target} Dist={dist:F1}m Bone={bone}");
        }
        ...
        public void DrawPanel()
        {
            ...
            for (int i = _logs.Count - 1; i >= 0; i--)
            {
                ...
                else if (l.Contains("[C]")) GUILayout.Label($"<color=magenta>{l}</color>");
                else if (l.Contains("[V]")) GUILayout.Label($"<color=lime>{l}</color>");
